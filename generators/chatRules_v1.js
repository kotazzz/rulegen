...
Для подробностей ознакомьтесь с полным списком правил ниже.`
   ...
   - Действия модерации: {{#case strictness}}{{#when low}}мут (12 часов){{/when}}{{#when medium}}мут (1 день){{/when}}{{#when high}}мут (3 дня), бан (при рецидиве){{/when}}{{/case}}.{{/if}}` // Added \n
        },
        {
            id: 'rule-spam',
            ...
            type: 'main-rule',
            defaultEnabled: true,
            textTemplate: `Спам и флуд: Запрещены спам, флуд, оффтоп и игнорирование тематики чата в любой форме. Это включает спам медиа, капсом, сообщениями без цели, злоупотребление пингами (@ упоминания), а также попытки помешать общению других в чате.\n{{#if general.showModerationActions}}
   - Действия модерации: {{#case strictness}}{{#when low}}варн, мут (6 часов){{/when}}{{#when medium}}варн, мут (12 часов), бан (при рецидиве){{/when}}{{#when high}}мут (1 день), бан (при рецидиве){{/when}}{{/case}}.{{/if}}` // Added \n
        },
        {
            id: 'rule-media',
            ...
            textTemplate: `Запрещенные медиа: Запрещена отправка медиа со сценами жестокого насилия, вирусов, вредоносного ПО, экстремистского или любого другого опасного для здоровья контента.\n{{#if general.showModerationActions}}
   - Действия модерации: {{#case strictness}}{{#when low}}мут (1 день){{/when}}{{#when medium}}мут (1 день), немедленный бан (при рецидиве){{/when}}{{#when high}}немедленный бан{{/when}}{{/case}}.{{/if}}` // Added \n
        },
         {
            ...
            textTemplate: `Запрещенные медиа (18+): Запрещена отправка медиа со сценами нелегального или чрезмерно жестокого контента (снафф, гуро и т.п.), вирусов, вредоносного ПО, экстремистских материалов. Контент 18+ должен быть помечен соответствующим образом (спойлер).\n{{#if general.showModerationActions}}
   - Действия модерации: {{#case strictness}}{{#when low}}мут (1 день){{/when}}{{#when medium}}мут (1 день), бан (при рецидиве){{/when}}{{#when high}}немедленный бан{{/when}}{{/case}}.{{/if}}` // Added \n
        },
        {
            id: 'rule-ads',
            ...
            textTemplate: `Реклама: Запрещена реклама любых ресурсов без согласования с администрацией. Это включает рекламу в личных сообщениях.\n{{#if general.showModerationActions}}
   - Действия модерации: {{#case strictness}}{{#when low}}варн, мут (12 часов){{/when}}{{#when medium}}варн, мут (24 часа), бан (при рецидиве){{/when}}{{#when high}}мут (2 дня), бан (при рецидиве){{/when}}{{/case}}.{{/if}}` // Added \n
        },
        {
            id: 'rule-ads-allowed',
            ...
            textTemplate: `Реклама: Реклама разрешена только с явного согласия администрации и в специально отведенных для этого местах/времени (если таковые имеются). Несогласованная реклама, в том числе в ЛС, запрещена.\n{{#if general.showModerationActions}}
   - Действия модерации (за несогласованную рекламу): {{#case strictness}}{{#when low}}варн, мут (6 часов){{/when}}{{#when medium}}варн, мут (12 часов), бан (при рецидиве){{/when}}{{#when high}}мут (1 день), бан (при рецидиве){{/when}}{{/case}}.{{/if}}` // Added \n
        },
        {
            id: 'rule-conflicts',
            ...
            type: 'main-rule',
            defaultEnabled: true,
            textTemplate: `Конфликты и провокации: Нельзя начинать или способствовать конфликту. Запрещены провокации по отношению как к участникам, так и к стаффу с целью проверки компетентности.\n{{#if general.showModerationActions}}
   - Действия модерации: {{#case strictness}}{{#when low}}варн, мут (12 часов){{/when}}{{#when medium}}варн, мут (24 часа), бан (при рецидиве){{/when}}{{#when high}}мут (2 дня), бан (при рецидиве){{/when}}{{/case}}.{{/if}}` // Added \n
        },
        {
            id: 'rule-bullying',
            ...
            type: 'main-rule',
            defaultEnabled: true,
            textTemplate: `Буллинг и троллинг: Запрещен буллинг и троллинг в любой форме в отношении участников и стаффа. Запрещено копирование и пародирование ников, профилей, выдача себя за другое лицо. Любые шутки могут быть восприняты модерацией всерьез.\n{{#if general.showModerationActions}}
   - Действия модерации: {{#case strictness}}{{#when low}}варн, мут (12 часов){{/when}}{{#when medium}}варн, мут (24 часа), бан (при рецидиве){{/when}}{{#when high}}мут (2 дня), бан (при рецидиве){{/when}}{{/case}}.{{/if}}` // Added \n
        },
        {
            id: 'rule-toxic',
            ...
            type: 'main-rule',
            defaultEnabled: true,
            textTemplate: `Токсичное поведение: Запрещено токсичное и неадекватное отношение к администрации и участникам.\n{{#if general.showModerationActions}}
   - Действия модерации: {{#case strictness}}{{#when low}}варн, мут (12 часов){{/when}}{{#when medium}}варн, мут (24 часа), бан (при рецидиве){{/when}}{{#when high}}мут (2 дня), бан (при рецидиве){{/when}}{{/case}}.{{/if}}` // Added \n
        },
        {
            id: 'rule-politics',
            ...
            type: 'main-rule',
            defaultEnabled: true,
            textTemplate: `Политика и нацистские шутки: Любые конфликты на политической основе в любом виде влекут за собой мут всех участников конфликта. Запрещены любые шутки, имеющие нацистский контекст.\n{{#if general.showModerationActions}}
   - Действия модерации: {{#case strictness}}{{#when low}}мут (12 часов){{/when}}{{#when medium}}мут (24 часа), бан (при рецидиве){{/when}}{{#when high}}мут (3 дня), бан (при рецидиве){{/when}}{{/case}}.{{/if}}` // Added \n
        },
        {
            id: 'rule-privacy',
            ...
            type: 'main-rule',
            defaultEnabled: true,
            textTemplate: `Конфиденциальность: Запрещено распространять личные данные других участников без их согласия.\n{{#if general.showModerationActions}}
   - Действия модерации: {{#case strictness}}{{#when low}}мут (12 часов){{/when}}{{#when medium}}мут (24 часа), бан (при рецидиве){{/when}}{{#when high}}мут (3 дня), бан (при рецидиве){{/when}}{{/case}}.{{/if}}` // Added \n
        },
        {
            id: 'rule-bypass',
            ...
            type: 'main-rule',
            defaultEnabled: true,
            textTemplate: `Обход правил: Запрещены любые попытки обхода правил.\n{{#if general.showModerationActions}}
   - Действия модерации: {{#case strictness}}{{#when low}}мут (24 часа){{/when}}{{#when medium}}бан (3 дня){{/when}}{{#when high}}перманентный бан{{/when}}{{/case}}.{{/if}}` // Added \n
        }
    ]
};
